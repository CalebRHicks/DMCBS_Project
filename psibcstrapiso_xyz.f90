...
contains
   ...
   integer(kind=i4) :: i,j,k,is,iter
   real(kind=r8) :: vext,dvext,d2vext,mat(3,3),vec(3),dum
   real(kind=r8) :: pi,rhor,mu,dr,integ,muloc,mumin,mumax
   real(kind=r8), parameter :: convergence=1e-6_r8
...
! assign spin 1 to up particles and -1 to down ones
   ...
   if (myrank().eq.0) then
      ...
      allocate(vorb(2*norb+2))
      do i=1,norb
         read (5,*) vorb(i) !amplitude
      enddo
      do i=norb+1,2*norb
         read (5,*) vorb(i) !factor in phi(r)
      enddo
      read (5,*) vorb(2*norb+1)
      read (5,*) vorb(2*norb+2)
      ...
      write (6,'(''b parameter of Jastrow ='',t40,f10.5)') b
   endif
   call bcast(omega)
   call bcast(nexup)
   call bcast(nexdn)
   ...
   call bcast(bex)
   call bcast(norb)
   if (myrank().ne.0) allocate(vorb(2*norb+2))
   call bcast(vorb)
   call bcast(radpair)
   call bcast(b)
   ...
   if (myrank().eq.0) then
      ...
      do i=1,norb
         ...
         write (6,'(''multiply omega by this'',t30,f20.10)') vorb(i+norb)
         do j=1,numsh(i)
            k=k+1
            write (6,'(''state '',t35,3i5)') nquant(:,k)
         enddo
      enddo
      write (6,'(''gamma1 = '',t40,f10.5)') vorb(2*norb+1)
      write (6,'(''gamma2 = '',t40,f10.5)') vorb(2*norb+2)
   endif
...
!     write(6,'(''LDA gamma1 ='',t40,f10.5)') vorb(2*norb+1)
!     write(6,'(''LDA gamma2 ='',t40,f10.5)') vorb(2*norb+2)
...
! NEW
   ...
   mumin=0.0_r8
   mumax=100.0_r8
   mu=0.5_r8*(mumax+mumin) ! first guess for mu
   iter=1
   dr=0.001_r8
   do while (iter.lt.itmax)
      ...
      do while (.true.)
         vext=0.5_r8*r**2
         muloc=(mu-vext)*2.0_r8/csi   ! m=hbar=1
         if (muloc.le.0.0_r8) exit
         rhor=muloc**(3.0_r8/2.0_r8)/(3.0_r8*pi**2)
         integ=integ+r**2*dr*rhor
         r=r+dr
      enddo
      integ=4.0_r8*pi*integ
      if (abs(integ-npart).lt.convergence) exit
      if ((integ-npart).lt.0.0_r8) then
         mumin=mu
      else
         mumax=mu
      endif
      mu=0.5_r8*(mumax+mumin)
      iter=iter+1
   enddo
   afact=2.0_r8/csi
   bfact=mu
   ...
   if (myrank().eq.0) then
      write(6,'(''LDA csi ='',t40,f10.5)') csi
      write(6,'(''LDA mu ='',t40,f10.5)') mu
      write(6,'(''LDA gamma1 ='',t40,f10.5)') vorb(2*norb+1)
      write(6,'(''LDA gamma2 ='',t40,f10.5)') vorb(2*norb+2)
      write(6,'(''LDA rmax ='',t40,f10.5)') rmax
      write(6,'(''LDA npart ='',t40,f10.5)') integ
   endif
! end NEW
   rmax=0.67_r8*rmax
...
!write (78,'(4f25.10)'),rcm,kf,dkf,d2kf
!enddo
!stop
...
!  if (kf.eq.0.0_r8) kf=1e-10_r8
   ...
   pf=phi*g1+f*g2
   dpf(1,:)=gradphi(1,:)*g1+phi*gradg1+gradf(1,:)*g2+f*gradg2
   dpf(2,:)=gradphi(2,:)*g1+phi*gradg1+gradf(2,:)*g2+f*gradg2
   d2pf(1)=delphi(1)*g1+2.0_r8*sum(gradphi(1,:)*gradg1(:))+phi*delg1 &
          +delf(1)*g2+2.0_r8*sum(gradf(1,:)*gradg2(:))+f*delg2
   d2pf(2)=delphi(2)*g1+2.0_r8*sum(gradphi(2,:)*gradg1(:))+phi*delg1 &
          +delf(2)*g2+2.0_r8*sum(gradf(2,:)*gradg2(:))+f*delg2
!write(99,*) rcm,r,pf
!enddo
!write(99,*) ' '
!write(99,*) ' '
...
! derivatives computed numerically
            do kk=1,numsh(ish)
               ...
               call getorb(nquant(:,k),omega*(vorb(ish+norb)+0.01),x(:,i),f1,df1,d2f1)
               call getorb(nquant(:,k),omega*(vorb(ish+norb)+0.01),x(:,jj),f2,df2,d2f2)
               pf1=pf1+vorb(ish)*f1*f2
               call getorb(nquant(:,k),omega*(vorb(ish+norb)-0.01),x(:,i),f1,df1,d2f1)
               call getorb(nquant(:,k),omega*(vorb(ish+norb)-0.01),x(:,jj),f2,df2,d2f2)
               pf2=pf2+vorb(ish)*f1*f2
            enddo
            dphi(i,j,np)=dphi(i,j,np)*g1
            dphi(i,j,np+norb)=(pf1-pf2)*g1/0.02_r8
            np=np+1
         enddo
         np=np+norb
         call radialfun(kf*r,f,dum1,dum2)
         dphi(i,j,np)=-pairf*rcm**2*g1
         np=np+1
         dphi(i,j,np)=-f*g2*rcm**2
         np=np+1
      enddo
   enddo
   do j=1,npdet
