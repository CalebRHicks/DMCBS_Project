module potential
   ...
   integer(kind=i4), private, save :: npart,ntab,ndim,ibox
   real(kind=r8), private, save :: hbar,v0,amu,el,range,scale,eli,rangev,scalev
...
contains
   ...
   if (myrank().eq.0) then
      ...
      if (djas.gt.0.5_r8) djas=0.5_r8*el
      ...
      write (6,'(''ibox - lattice sum for potential ='',t40,i10)') ibox
...
! use an effective interaction for Jastrow
   ...
   if (myrank().eq.0) then
      do i=0,ntab
         ...
         if (myrank().eq.0) write(79,'(f10.5,e15.7)') i/scalev,vtab(1,i)
      enddo
   endif
...
!vpot=-2.0_r8*v0*amu**2*hbar*exp(-0.5_r8*(amu*r)**2)
   ...
   integer(kind=i4) :: is,i,j,index,itab,ix,iy,iz
   real(kind=r8), dimension(ndim) :: dx,dx0
      ...
      do j=i+1,npart
         if (ieq.and.spin(i).eq.spin(j).or.iop.and.spin(i).ne.spin(j)) then
            ...
            if (r.lt.range) then
               ...
               u=u+uj
               du(:,i)=du(:,i)+dx0(:)*duj/(r*sqrt(mass(i)))
               du(:,j)=du(:,j)-dx0(:)*duj/(r*sqrt(mass(j)))
