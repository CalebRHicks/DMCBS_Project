module slaterbcs
   ...
   real(kind=r8), private, save :: qex(3)
   real(kind=r8), private, save :: v0ex,v0exjas
contains
   ...
   real(kind=r8) :: pi
   integer(kind=i4) :: nqex(3),numq
   real(kind=r8) :: jbeta,jgamma
   npart=npartin
   ndim=ndin
   allocate(nbin(2))
   nbin=nbinin
...
! careful, the next few things only work in 3D, FIX IT !!!
      write (6,'(''nqex ='',t40,3i5)') nqex
      v0ex=v0ex*0.5_r8  ! v0 times E_F
      write (6,'(''v0ex*EF ='',t40,f10.5)') v0ex
      write (6,'(''v0 jastrow ='',t40,f10.5)') v0exjas
      v0exjas=v0exjas*v0ex  ! v0 times E_F
      write (6,'(''long range Jastrow: number of q vectors'',t40,i10)') numq
      write (6,'(''long range Jastrow: beta parameter'',t40,f10.5)') jbeta
      write (6,'(''long range Jastrow: gamma parameter'',t40,f10.5)') jgamma
   endif
   call bcast(rho)
   call bcast(el)
   call bcast(nexup)
...
! initialize s-wave pairing function
   ...
   qex=nqex*2.0_r8*pi/el
   call setjaslong(npart,el,numq,jbeta,jgamma)
...
! I must have storaged smati
   ...
   subroutine bcsonebody(x,g,dg,d2g,vext)
   real(kind=r8) :: x(:,:),g,dg(:,:),d2g,vext
   real(kind=r8) :: arg,c,s
   integer(kind=i4) :: i
   vext=0.0_r8
   g=0.0_r8
   dg=0.0_r8
   ...
   do i=1,npart
      ...
      g=g-v0exjas*c
      dg(:,i)=v0exjas*qex(:)*s
      d2g=d2g+v0exjas*sum(qex**2)*(v0exjas*s**2+c)
   enddo
   end subroutine bcsonebody
