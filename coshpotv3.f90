module potential
   ...
   real(kind=r8), private, save :: hbar,v0,amu,el,range,scale,eli,v3
...
contains
   ...
   if (myrank().eq.0) then
      ...
      write (6,'(''V3*hbar*amu**2/2mu of three-body interaction ='',t40,f10.5)') v3*hbar*amu**2
...
!vpot=-2.0_r8*v0*amu**2*hbar*exp(-0.5_r8*(amu*r)**2)
   ...
   is=1
   d2u=d2u+sum(du*du)
! three-body interaction between the light particles and heavies
   v3pot=0.0_r8
   do i=2,npart-1
      dx(:)=x(:,1)-x(:,i)
      dx=dx-el*nint(dx*eli)
      ...
      if (r.lt.range) then
         ...
         c3=-(dr+1.0_r8)*dr*(dr-2.0_r8)/2.0_r8
         c4=(dr+1.0_r8)*dr*(dr-1.0_r8)/6.0_r8
         v3p=c1*vtab(1,index-1)+c2*vtab(1,index) &
             +c3*vtab(1,index+1)+c4*vtab(1,index+2)
         do j=i+1,npart
            dx(:)=x(:,1)-x(:,j)
            dx=dx-el*nint(dx*eli)
            r=max(tiny,sqrt(sum(dx**2)))
            if (r.lt.range) then
               ...
               c3=-(dr+1.0_r8)*dr*(dr-2.0_r8)/2.0_r8
               c4=(dr+1.0_r8)*dr*(dr-1.0_r8)/6.0_r8
               v3pp=c1*vtab(1,index-1)+c2*vtab(1,index) &
                   +c3*vtab(1,index+1)+c4*vtab(1,index+2)
               v3pot=v3pot+v3*v3p*v3pp
            endif
         enddo
      endif
   enddo
