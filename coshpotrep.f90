...
! use an effective interaction for Jastrow
   ...
   if (cjas(1).ne.0.0_r8) then
      ...
      v0=v0op
      call orbital(f,df,d2f,hbar,ntab0,dr,vpot,1)
   ...
   real(kind=r8) :: v,u,d2u,vpot,uj,duj,d2uj,d2uu(npart)
   integer(kind=i4) :: is,i,j,index,itab,k
      ...
      do j=i+1,npart
         if (ieq.and.spin(i).eq.spin(j).or.iop.and.spin(i).ne.spin(j)) then
            ...
            if (r.lt.range) then
               ...
               v=v+vpot
               u=u*uj
               du(:,i)=du(:,i)+duj*dx/(r*uj)
               du(:,j)=du(:,j)-duj*dx/(r*uj)
               d2uu(i)=d2uu(i)+(d2uj+2.0_r8*duj/r)/uj-(duj/uj)**2
               d2uu(j)=d2uu(j)+(d2uj+2.0_r8*duj/r)/uj-(duj/uj)**2
            endif
         endif
      enddo
   enddo
